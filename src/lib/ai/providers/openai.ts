...
export async function analyze(req: UniversalAIRequest): Promise<UniversalAIResponse> {
  ...
  try {
    ...
    const response = await openai.chat.completions.create({ model: currentModel, messages: [{ role: "user", content: messagesContent }] });
    if (!response?.choices?.length) throw new Error(`OpenAI Model ${currentModel} returned an invalid response.`);
  ...
  } catch (error: any) { console.warn(`OpenAI model failed: ${currentModel} - ${error.message}`); throw error; }
}
