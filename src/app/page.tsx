...
function MTFCountdown({
  ...
  if (timeLeft === "Ready") {
    return (
      <button
        onClick={onReset}
        className="text-amber-500 hover:text-amber-400 underline text-[10px] ml-2"
      >
        Recapture
      </button>
    );
...
export default function Dashboard() {
  ...
  const [showStopObservationModal, setShowStopObservationModal] =
    useState(false);
  ...
  const handleDeleteTrades = async (localIds?: string[]) => {
    if (!confirm(localIds ? "Delete selected trades?" : "Delete ALL trades?"))
      return;
    try {
      setIsDeleting(true);
      const dbIds = localIds
        ? useTradingStore
            .getState()
            .tradeHistory.filter((t) => localIds.includes(t.id) && t.dbTradeId)
            .map((t) => t.dbTradeId!)
        : [];
      ...
      if (localIds) {
        useTradingStore.setState((state) => ({
          tradeHistory: state.tradeHistory.filter(
            (t) => !localIds.includes(t.id),
          ),
        }));
        setSelectedTrades([]);
      } else {
  ...
  useEffect(() => {
    if (apiFailCount >= 3 && apiFailCount % 3 === 0) {
      toast.error(
        "API is failing repeatedly. Please switch your AI Provider from the top right settings.",
        {
          duration: 5000,
        },
      );
  ...
  if (marketDataMode === "visual_only") {
    expectedFrames = calculateExpectedFrames(
      timeframe,
      tradeDuration,
      observationFrequency,
    );
  ...
  useEffect(() => {
    ...
    if (
      state.analysisSessionKey !== null &&
      state.analysisSessionKey !== newKey
    ) {
    ...
    selectedProvider,
    selectedModel,
    useTradingStore.getState().activeConnectionId,
    observationFrequency,
  ]);
  ...
  useEffect(() => {
    ...
    const runProgressiveAnalysis = async () => {
      ...
      if (
        initial.isProgressiveAnalyzing ||
        initial.isFetchingAnalysis ||
        !initial.isLiveObservationEnabled ||
        initial.isLiveObservationPaused
      )
        return;
      ...
      try {
        while (true) {
          ...
          if (
            ...
            current.isLiveObservationPaused
          ) {
            break;
          }
          ...
          try {
            const response = await fetch("/api/progressive-analyze", {
              ...
              body: JSON.stringify({
                ...
                macroTimeframeImage: current.macroTimeframeImage
                  ? { timeframe: "4h", image: current.macroTimeframeImage }
                  : undefined,
                confirmationTimeframeImage: current.confirmationTimeframeImage
                  ? {
                      timeframe: "1h",
                      image: current.confirmationTimeframeImage,
                    }
                  : undefined,
                structureTimeframeImage: current.structureTimeframeImage
                  ? { timeframe: "15m", image: current.structureTimeframeImage }
                  : undefined,
                primaryTimeframe: {
                  timeframe: "5m",
                  screenshots: await Promise.all(
                    framesToAnalyze.map(async (obs) => ({
                      timestamp: new Date(obs.timestamp).toISOString(),
                      mimeType: "image/jpeg",
                      // base64: obs.imageBase64 || (obs.imageId ? await ImageStore.getImage(obs.imageId) : ""),
                      base64: obs.imageId
                        ? await ImageStore.getImage(obs.imageId)
                        : "",
                      platform: current.platform,
                      symbol: current.symbol,
                    })),
            ...
            if (
              data?.marketState?.includes("SETUP_FORMING") &&
              latest.observationFrequency > 30
            ) {
              latest.setObservationFrequency(30);
              toast.success(
                "SETUP_FORMING detected. Accelerating analysis to 30s intervals.",
                { icon: "⚡" },
              );
            }
            ...
            toast.error(
              "Live Observation batch failed. Please retry when ready.",
            );
  ...
  const handleCaptureMTF = (timeframe: "4h" | "1h" | "15m") => {
    ...
    if (timeframe === "4h") {
      store.setMacroTimeframeImage(imageBase64);
      store.setMacroTimeframeCapturedAt(Date.now());
    } else if (timeframe === "1h") {
      store.setConfirmationTimeframeImage(imageBase64);
      store.setConfirmationTimeframeCapturedAt(Date.now());
    } else if (timeframe === "15m") {
      store.setStructureTimeframeImage(imageBase64);
      store.setStructureTimeframeCapturedAt(Date.now());
    }

    toast.success(`${timeframe.toUpperCase()} Context Captured!`, {
      icon: "📸",
    });
  };
  ...
  const handleAnalyzeSnapshot = async () => {
    ...
    const unanalyzedCount =
      storeState.observations.length -
      (storeState.lastAnalyzedObservationIndex === -1
        ? 0
        : storeState.lastAnalyzedObservationIndex + 1);
    ...
    if (
      storeState.isLiveObservationEnabled &&
      storeState.useDualModel &&
      unanalyzedCount > 0 &&
      unanalyzedCount < 20
    ) {
      toast.loading("Analyzing partial batch before final signal...", {
        id: "partial_batch",
      });
      const startIndex =
        storeState.lastAnalyzedObservationIndex === -1
          ? 0
          : storeState.lastAnalyzedObservationIndex + 1;
      const partialFrames = storeState.observations.slice(
        startIndex,
        startIndex + unanalyzedCount,
      );
      ...
      try {
        const progressiveResponse = await fetch("/api/progressive-analyze", {
          ...
          body: JSON.stringify({
            ...
            macroTimeframeImage: storeState.macroTimeframeImage
              ? { timeframe: "4h", image: storeState.macroTimeframeImage }
              : undefined,
            confirmationTimeframeImage: storeState.confirmationTimeframeImage
              ? {
                  timeframe: "1h",
                  image: storeState.confirmationTimeframeImage,
                }
              : undefined,
            structureTimeframeImage: storeState.structureTimeframeImage
              ? { timeframe: "15m", image: storeState.structureTimeframeImage }
              : undefined,
            primaryTimeframe: {
              timeframe: "5m",
              screenshots: await Promise.all(
                partialFrames.map(async (obs) => ({
                  timestamp: new Date(obs.timestamp).toISOString(),
                  mimeType: "image/jpeg",
                  base64: obs.imageId
                    ? await ImageStore.getImage(obs.imageId)
    ...
    try {
      const reqBody: any = {
        ...
        macroTimeframeImage: useTradingStore.getState().macroTimeframeImage
          ? {
              timeframe: "4h",
              image: useTradingStore.getState().macroTimeframeImage,
            }
          : undefined,
        confirmationTimeframeImage: useTradingStore.getState()
          .confirmationTimeframeImage
          ? {
              timeframe: "1h",
              image: useTradingStore.getState().confirmationTimeframeImage,
            }
          : undefined,
        structureTimeframeImage: useTradingStore.getState()
          .structureTimeframeImage
          ? {
              timeframe: "15m",
              image: useTradingStore.getState().structureTimeframeImage,
            }
          : undefined,
      };
        ...
        if (obs.length > 0) {
          ...
          if (obs.length <= maxImages) {
            ...
            const uniqueTs = Array.from(
              new Set(selected.map((s) => s.timestamp)),
            );
            uniqueTs.sort((a, b) => a - b);
            selected.length = 0;
            selected.push(
              ...uniqueTs.map((ts) => obs.find((o) => o.timestamp === ts)!),
            );
          }

          reqBody.screenshots = await Promise.all(
            selected.map(async (o) => ({
              timestamp: new Date(o.timestamp).toISOString(),
              mimeType: "image/jpeg",
              base64:
                o.imageBase64 ||
                (o.imageId ? await ImageStore.getImage(o.imageId) : ""),
              platform: platform,
              symbol: symbol,
            })),
          );
      ...
      if (res.ok) {
        ...
        if (data.marketDataStatus === "fallback") {
          toast(
            "Live data not found for this symbol. Falling back to Visual-Only mode.",
            {
              icon: "👀",
            },
          );
      ...
      const response = await fetch("/api/progressive-analyze", {
        ...
        body: JSON.stringify({
          ...
          macroTimeframeImage: current.macroTimeframeImage
            ...
            : undefined,
          confirmationTimeframeImage: current.confirmationTimeframeImage
            ? { timeframe: "1h", image: current.confirmationTimeframeImage }
            : undefined,
          structureTimeframeImage: current.structureTimeframeImage
            ? { timeframe: "15m", image: current.structureTimeframeImage }
            : undefined,
    ...
    <main className="container mx-auto p-4 md:p-6 lg:p-8 max-w-7xl">
      ...
      <header className="flex flex-col gap-6 mb-6">
        <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-4">
          <div className="flex items-center justify-between w-full">
            <div>
              ...
              <p className="text-sm md:text-base text-zinc-400">
                Live AI chart analysis and probability-based signals
              </p>
            </div>
            ...
            <Button
              ...
              className="bg-zinc-900 border-zinc-800 hover:bg-zinc-800 text-zinc-300 gap-2 shadow-sm"
            >
          ...
          {isConfigOpen && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 bg-zinc-950 border border-zinc-800/50 rounded-xl p-5 animate-in slide-in-from-top-2 fade-in duration-200 shadow-xl w-full">
                ...
                <TooltipProvider>
                  <div className="flex flex-col space-y-1.5">
                    <div className="flex items-center gap-1">
                      <Label className="text-[9px] text-zinc-500 uppercase tracking-wider font-semibold">
                        {useTradingStore.getState().useDualModel
                          ? "Progressive Analysis Model (Vision)"
                          : "AI Model"}
                      </Label>
                      <Tooltip>
                        <TooltipTrigger>
                        ...
                        <TooltipContent className="bg-zinc-900 border-zinc-800 text-zinc-200">
                          <p>
                            {useTradingStore.getState().useDualModel
                              ? "This model is used ONLY by: /api/progressive-analyze"
                              : "Select which AI provider and model will analyze your chart."}
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    </div>
                    ...
                    >
                      <SelectTrigger className="w-full h-9 text-xs bg-zinc-900 border-zinc-800 text-zinc-200 focus:ring-0 focus:ring-offset-0">
                  ...
                  <div className="flex flex-col space-y-1.5 mt-4">
                    <div className="flex items-center gap-1">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="bg-zinc-900 border-zinc-800 text-zinc-200">
                          <p>
                            Enable Dual Model mode to save tokens. Vision model
                            extracts data, Text model analyzes it.
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    </div>
                    <div className="flex items-center gap-2 h-8">
                      <Switch
                        checked={useTradingStore.getState().useDualModel}
                        onCheckedChange={(val) =>
                          useTradingStore.getState().setUseDualModel(val)
                        }
                  ...
                  {useTradingStore.getState().useDualModel && (
                    <div className="flex flex-col space-y-1.5 mt-4">
                      <div className="flex items-center gap-1">
                        <Label className="text-[9px] text-amber-500 uppercase tracking-wider font-semibold">
                          Final Analysis Model (Text)
                        </Label>
                        <Tooltip>
                          <TooltipTrigger>
                            <Info className="w-3 h-3 text-zinc-600 cursor-help" />
                      ...
                      <Select
                        value={`${useTradingStore.getState().selectedReasoningProvider}:${useTradingStore.getState().selectedReasoningModel}`}
                        onValueChange={(val) => {
                          if (!val) return;
                          const idx = val.indexOf(":");
                          const provider = val.substring(0, idx) as any;
                          ...
                          useTradingStore
                            .getState()
                            .setSelectedReasoningModel(provider, model);
                        }}
                      >
                        <SelectTrigger className="w-full h-9 text-xs bg-zinc-900 border-zinc-800 text-zinc-200 focus:ring-0 focus:ring-offset-0">
                          <Settings2 className="w-3 h-3 mr-2 text-zinc-400" />
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <Select
                    value={useTradingStore
                      .getState()
                      .selectedStrategies.join(",")}
                    onValueChange={(val: string | null) => {
                      if (!val) return;
                      const current =
                        useTradingStore.getState().selectedStrategies;
                      if (current.includes(val)) {
                        ...
                        useTradingStore
                          .getState()
                          .setSelectedStrategies([...current, val]);
                      }
                    }}
                  >
                    <SelectTrigger className="w-full h-9 text-xs bg-zinc-900 border-zinc-800 text-zinc-200 focus:ring-0 focus:ring-offset-0">
                ...
                <div className="flex flex-col space-y-1.5 mt-4">
                  ...
                  <Select
                    ...
                    onValueChange={(val: any) =>
                      useTradingStore.getState().setTradingMode(val)
                    }
                  >
                    <SelectTrigger className="w-full h-9 text-xs bg-zinc-900 border-zinc-800 text-zinc-200 focus:ring-0 focus:ring-offset-0">
              ...
              <div className="space-y-4">
                ...
                {marketDataMode === "visual_only" && (
                  <div className="flex flex-col space-y-1.5">
                    ...
                    <Select
                      value={useTradingStore
                        .getState()
                        .visibleIndicators.join(",")}
                      onValueChange={(val: string | null) => {
                        if (!val) return;
                        const current =
                          useTradingStore.getState().visibleIndicators;
                        if (current.includes(val)) {
                          useTradingStore
                            .getState()
                            .setVisibleIndicators(
                          ...
                          useTradingStore
                            .getState()
                            .setVisibleIndicators([...current, val]);
                        }
                      }}
                    >
                      <SelectTrigger className="w-full h-9 text-xs bg-zinc-900 border-zinc-800 text-zinc-200 focus:ring-0 focus:ring-offset-0">
                ...
                <div className="flex flex-col space-y-1.5 mt-4">
                  ...
                  <Select
                    value={observationFrequency.toString()}
                    onValueChange={(val: string | null) => {
                      if (val) setObservationFrequency(parseInt(val));
                    }}
                  >
                    <SelectTrigger className="w-full h-9 text-xs bg-zinc-900 border-zinc-800 text-zinc-200 focus:ring-0 focus:ring-offset-0">
                ...
                {marketDataMode === "api" && (
                  <div className="flex flex-col justify-end mt-4 pt-4 border-t border-zinc-800/50">
                    <SettingsDialog />
                  </div>
                )}
              </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 bg-zinc-950 border border-zinc-800/50 rounded-lg p-3">
            ...
            {!isAnalyzing ? (
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  ...
                  disabled={
                    isFetchingAnalysis ||
                    (marketDataMode === "visual_only" && isAnalyzeDisabled)
                  }
                ...
                <Button
                  ...
                  variant="secondary"
                  className="text-zinc-300 h-8 text-xs px-3"
                >
                  Clear
                </Button>
                <Button
            ...
            <div className="flex-1 flex flex-col md:flex-row items-start md:items-center gap-4 lg:border-l lg:border-zinc-800 lg:pl-4 justify-end">
                      ...
                      {useTradingStore.getState().macroTimeframeCapturedAt ? (
                        <MTFCountdown
                          ...
                          onReset={() => {
                            useTradingStore
                              .getState()
                              .setMacroTimeframeImage(null);
                            useTradingStore
                              .getState()
                              .setMacroTimeframeCapturedAt(null);
                          }}
                        />
                      ) : (
                        <button
                  ...
                  {useTradingStore.getState().confirmationTimeframeImage ? (
                    <div className="flex items-center">
                      ...
                      {useTradingStore.getState()
                        .confirmationTimeframeCapturedAt ? (
                        <MTFCountdown
                          capturedAt={
                            useTradingStore.getState()
                              .confirmationTimeframeCapturedAt!
                          ...
                          onReset={() => {
                            useTradingStore
                              .getState()
                              .setConfirmationTimeframeImage(null);
                            useTradingStore
                              .getState()
                              .setConfirmationTimeframeCapturedAt(null);
                          }}
                        />
                      ) : (
                        <button
                      ...
                      {useTradingStore.getState()
                        ...
                        <MTFCountdown
                          ...
                          onReset={() => {
                            useTradingStore
                              .getState()
                              .setStructureTimeframeImage(null);
                            useTradingStore
                              .getState()
                              .setStructureTimeframeCapturedAt(null);
                          }}
                        />
                      ) : (
                        <button
              ...
              <div className="flex flex-col text-[10px] text-zinc-400 min-w-[140px] space-y-1">
                ...
                {!isLiveObservationEnabled ? (
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <Button
                      onClick={() =>
                        setIsLiveObservationPaused(!isLiveObservationPaused)
                      }
                      variant="outline"
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="text-zinc-200">
                    {useTradingStore.getState().totalFramesCaptured}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="text-zinc-200">
                    {useTradingStore.getState().progressiveAnalyses.length}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col text-[10px] text-zinc-400 md:border-l md:border-zinc-800 md:pl-4 min-w-[140px] space-y-1">
                <div className="flex items-center justify-between gap-4 mb-2">
                  ...
                  <span className="text-zinc-200">
                    {Math.ceil(timeUntilNextFrame).toString().padStart(2, "0")}s
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span
                    className={`font-medium ${
                      analysisReadiness.includes("EXCELLENT") ||
                      analysisReadiness.includes("COMPLETE") ||
                      analysisReadiness.includes("VERY GOOD")
                        ? "text-green-400"
...

                {
                  <div className="flex items-center justify-between gap-4 mt-1">
                    ...
                    <span
                      className="text-zinc-200 truncate max-w-[80px]"
                      title={
                        useTradingStore.getState().progressiveAnalyses.length >
                        0
                          ? `Batch #${
                              useTradingStore.getState().progressiveAnalyses[
                                useTradingStore.getState().progressiveAnalyses
                                  .length - 1
                              ].batchId
                            }`
          ...
          {useTradingStore.getState().retryProgressiveAnalysis &&
            marketDataMode === "visual_only" &&
            stream && (
              <div className="flex justify-end px-4 pb-4">
                ...
                >
                  {useTradingStore.getState().retryLoading
                    ? "Retrying..."
                    : "Retry Progressive Analysis"}
                </button>
              </div>
            )}
        </div>
            ...
            {marketDataMode === "api" && (
              <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-zinc-400 z-10 backdrop-blur-sm">
                ...
                <p className="font-medium text-lg text-white">
                  API Data Mode Active
                </p>
                <p className="text-sm mt-2 text-center max-w-sm">
                  Visual chart capture is disabled. Analysis is running entirely
                  on live programmatic data from your Exchange Connection.
                </p>
              </div>
            )}
            {!stream && marketDataMode === "visual_only" ? (
              <div className="flex-1 flex flex-col items-center justify-center text-zinc-500">
                ...
                <p className="text-sm">
                  Click "Connect Chart" to select a browser tab.
                </p>
              </div>
            ) : (
              <video
              ...
              <div className="absolute top-4 right-4 flex gap-2 z-20 bg-black/60 p-2 rounded-lg backdrop-blur-sm border border-white/10">
                  ...
                  <Input
                    style={{
                      ...
                      border:
                        ...
                        isFieldInvalid(useTradingStore.getState().platform)
                          ? "1px solid #ef4444"
                    ...
                    onChange={(e) =>
                      useTradingStore.getState().setPlatform(e.target.value)
                    }
                    placeholder="e.g. Binomo"
                  />
                  {showValidationErrors &&
                    isFieldInvalid(useTradingStore.getState().platform) && (
                      <span className="text-[9px] text-red-500 absolute -bottom-4 left-1 font-bold">
                        Required
                      </span>
                    )}
                </div>
                ...
                <div className="flex flex-col gap-1 relative">
                  ...
                  <Input
                    ...
                    value={timeframe}
                    onChange={(e) =>
                      useTradingStore.getState().setTimeframe(e.target.value)
                    }
                    placeholder="e.g. 5m"
                  />
                  {showValidationErrors && isFieldInvalid(timeframe) && (
                ...
                <div className="flex flex-col gap-1 relative">
                  ...
                  <Input
                    style={{
                      ...
                      border:
                        showValidationErrors &&
                        isFieldInvalid(useTradingStore.getState().tradeDuration)
                          ? "1px solid #ef4444"
                          : "1px solid rgba(255,255,255,0.1)",
                      boxShadow: "0 4px 12px rgba(0,0,0,0.5)",
                    }}
                    ...
                    onChange={(e) =>
                      useTradingStore
                        .getState()
                        .setTradeDuration(e.target.value)
                    }
                    placeholder="e.g. 5m"
                  />
                  {showValidationErrors &&
                    isFieldInvalid(
                      useTradingStore.getState().tradeDuration,
                    ) && (
                      <span className="text-[9px] text-red-500 absolute -bottom-4 left-1 font-bold">
                        Required
                      </span>
          ...
          <Card className="glass-card border-none">
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                {useTradingStore.getState().requestedIndicators &&
                  useTradingStore.getState().requestedIndicators!.length >
                    0 && (
                    <div className="mb-3 p-3 bg-orange-500/10 border border-orange-500/30 rounded-md text-orange-400 text-sm">
                      <strong>AI Request:</strong> I need you to add the
                      following indicators to your chart for confirmation before
                      I can give a valid signal: <br />
                      {useTradingStore
                        .getState()
                        .requestedIndicators!.join(", ")}
                    </div>
                  )}
                <p className="text-sm text-zinc-300 bg-black/40 p-3 rounded-md border border-white/5">
                  {explanation ||
                    "Awaiting chart data to analyze market structure and indicators..."}
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card border-none">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Calculator className="w-5 h-5 text-purple-400" /> Risk
                Management
              </CardTitle>
            </CardHeader>
          ...
          <Card className="glass-card border-none">
            <CardHeader className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <CardDescription>
                  History of AI recommendations and market data
                </CardDescription>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <Select
                  value={filterSignal}
                  onValueChange={(val) => setFilterSignal(val || "ALL")}
                >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left text-zinc-300">
                    ...
                    <tr>
                      <th className="px-4 py-3 w-[40px]">
                        <input
                          ...
                          checked={
                            tradeHistory.length > 0 &&
                            selectedTrades.length === tradeHistory.length
                          }
                          onChange={(e) => {
                            if (e.target.checked) {
                  ...
                  <tbody>
                    {tradeHistory
                      .filter((t: any) => {
                        if (filterSignal !== "ALL" && t.signal !== filterSignal)
                          return false;
                          ...
                          <td className="px-4 py-3 font-mono text-xs">
                            {trade.entryPrice ? (
                              <div className="flex gap-2">
                                ...
                                {trade.entryPrice}
                                <span className="text-red-400/80">S:</span>
                                {trade.stopLoss}
                                <span className="text-green-400/80">T:</span>
                                {trade.takeProfit}
                              </div>
                            ) : (
                              "-"
                          ...
                          <td className="px-4 py-3 font-mono text-xs">
                            <div className="flex gap-2">
                              <span className="text-green-400/80">
                                +{trade.maxFavorableMove?.toFixed(2) || "0.00"}
                              </span>
                              <span className="text-red-400/80">
                                -{trade.maxAdverseMove?.toFixed(2) || "0.00"}
                              </span>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            ...
                            trade.signal !== "UNSURE" ? (
                              <TradeCountdown
                                compact
                                startedAt={
                                  trade.paperTradeStartedAt || trade.timestamp
                                }
                                duration={
                                  trade.tradeDuration || trade.timeframe || "5m"
                                }
                              />
                            ) : (
      ...
      <Dialog
        open={showStopObservationModal}
        onOpenChange={setShowStopObservationModal}
      >
        <DialogContent className="sm:max-w-[425px] bg-zinc-900 border-zinc-800 text-zinc-100">
          <DialogHeader>
            ...
            <DialogDescription className="text-zinc-400">
              Stopping Live Observation will clear the current live observation
              data.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            ...
            <p className="text-sm text-zinc-400 mt-2">
              Pending progressive analysis data will also be cleared. This data
              cannot be recovered.
            </p>
          </div>
